...
import { aiAnswer, buildAlerts, buildQuickReport } from "@/lib/utils/analytics";
...
const reportButtons = [
  { label: "Reporte diario", type: "daily" },
  { label: "Mensaje bloqueado", type: "blocked-message" },
  { label: "Firma pendiente", type: "signature-message" },
  { label: "Retail pendiente", type: "retail" },
  { label: "Movilidad pendiente", type: "mobility" },
  { label: "Prioridades", type: "priorities" },
];
...
export function AiCopilot({ data }: { data: OnboardingData }) {
  ...
  return (
    <section id="copiloto-ia" className="overflow-hidden rounded-[2.4rem] border border-[var(--border)] bg-[linear-gradient(135deg,var(--surface),var(--surface-soft))] p-5 shadow-[var(--shadow)] transition duration-300 hover:shadow-2xl lg:p-7">
      <div className="grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
        <div>
          ...
          <div className="mt-5 grid gap-3 md:grid-cols-3">
            ...
            <Mini title="Acciones IA" value="16" />
          </div>
        </div>
        <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface-strong)] p-4">
          ...
          <button onClick={() => setAnswer(aiAnswer(prompt, data))} className="mt-3 w-full rounded-2xl bg-[var(--brand)] px-4 py-3 font-black text-white shadow-[0_18px_45px_rgba(230,0,0,.28)] transition hover:-translate-y-0.5">Ejecutar análisis IA</button>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-[1fr_420px]">
        ...
        <div className="space-y-4">
          <div className="rounded-[2rem] border border-[var(--border)] bg-[var(--surface-strong)] p-5"><h3 className="text-xl font-black">Reportes rápidos</h3><div className="mt-4 grid grid-cols-2 gap-2">{reportButtons.map((button) => <button key={button.type} onClick={() => setAnswer(buildQuickReport(button.type, data))} className="rounded-2xl border border-[var(--border)] bg-[var(--surface-soft)] px-3 py-2 text-xs font-black transition hover:border-[var(--brand)] hover:text-[var(--brand)]">{button.label}</button>)}</div></div>
